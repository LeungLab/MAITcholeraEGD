...
#Clean up the data to get only those data which has TRBJ sequence
...
new_data2<-new_data1[,c(26,2)]
new_data2<-new_data2 %>% filter(Patient_info != "NANANA", TRBJ != "NA")

#Count and find the proportion of each TRBJ sequence for each patient


test<- count(new_data2,Patient_info,TRBJ)

final500<-acast(test, TRBJ~Patient_info, value.var = "n", fun.aggregate = sum)
