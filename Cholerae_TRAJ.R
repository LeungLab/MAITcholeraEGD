...
#Clean up the data to get only those data which has TRAJ sequence
...
new_data2<-new_data1[,c(26,5)]
new_data2<-new_data2 %>% filter(Patient_info != "NANANA", TRAJ != "NA")

#Count and find the proportion of each TRAJ sequence for each patient
test<- count(new_data2,Patient_info,TRAJ)

final500<-acast(test, TRAJ~Patient_info, value.var = "n", fun.aggregate = sum)
